...
var cuisineTypeSelect = $('select#cuisineType-select');
...
async function getFoodCards() {
    ...
    if ($(cuisineTypeSelect).val() != "All") {
        dataParam["cuisineType"] = $(cuisineTypeSelect).val();
    }

    if ($(inputSearchBar).val() != "") {
        dataParam["q"] = $(inputSearchBar).val();
    }
...
$(searchBtn).on('click', (e) => {
    ...
    if (isAlreadyCalledOnce == false) {
        $("div#no-more-results").remove();

        $('div.content-page div.foodview > *').remove();
        getFoodCards();
    }
